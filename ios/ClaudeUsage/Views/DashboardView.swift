...
struct DashboardView: View {
    ...
    private func usageContent(_ data: UsageData) -> some View {
        VStack(spacing: 12) {
            ...
            if !SharedStore.shared.usingAppGroup {
                widgetHint
            }
        }
    }
    ...
    /// Shown only when the App Group container is unavailable (bare free-signed build): the app
    /// works fully, but the home-screen widget can't read this cache. Explains why, once.
    private var widgetHint: some View {
        Text("Widget sharing is off because this build isn't using an App Group. The app works fully; see the README to enable the home-screen widget.")
            .font(.caption)
